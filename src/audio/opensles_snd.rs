    ...
    pub mod api {
        ...
        impl StreamProperties {
            pub fn num_channels(&self) -> usize {
                (self.channels | channel_mask::FRONT_LEFT) as usize
                    + (self.channels | channel_mask::FRONT_RIGHT) as usize
                    + (self.channels | channel_mask::FRONT_CENTER) as usize
            }
        }
...
unsafe fn audio_thread(rx: mpsc::Receiver<AudioMessage>) {
    use opensles::api::*;
...
impl AudioContext {
    pub fn new() -> AudioContext {
        ...
        std::thread::spawn(move || unsafe { audio_thread(rx) });

        AudioContext { tx, id: 1 }
    }
}
